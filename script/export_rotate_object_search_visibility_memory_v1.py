...
TASK_TYPE_NAME = "object_search_visibility_memory_v1"
DEFAULT_OUTPUT_DIR_NAME = "vlm_object_search_visibility_memory_v1"
...
def _history_selection_indices(
    ...
    if history_limit <= 0 or current_rel_idx <= 0:
        return []
    candidate_indices = list(range(0, current_rel_idx))
    if len(candidate_indices) <= history_limit:
        return candidate_indices

    selected: list[int] = []

    def _keep(rel_idx: int | None) -> None:
        if rel_idx is None:
            return
        value = int(rel_idx)
        if value < 0 or value >= current_rel_idx:
            return
        if value not in selected:
            selected.append(value)

    _keep(0)
    _keep(_latest_target_history_rel_idx(subtask_annotations, current_rel_idx, target_key))
    for rel_idx in range(max(0, current_rel_idx - 4), current_rel_idx):
        _keep(rel_idx)

    remaining = history_limit - len(selected)
    if remaining > 0:
        pool = [idx for idx in candidate_indices if idx not in selected]
        if len(pool) <= remaining:
            selected.extend(pool)
        else:
            stride = float(len(pool) - 1) / float(max(remaining - 1, 1))
            for step in range(remaining):
                pool_idx = int(round(step * stride))
                selected.append(pool[min(pool_idx, len(pool) - 1)])

    return sorted(selected[-history_limit:])
...
def _episode_samples_payload(
    ...
    for span_start, span_end in _iter_subtask_spans(annotations):
        ...
        for current_rel_idx, current_annotation in enumerate(subtask_annotations):
            raw_stage = int(current_annotation.get("stage", 0) or 0)
            if raw_stage >= 3:
                continue

            target_key = _annotation_target_key(current_annotation)
            history_rel_indices = _history_selection_indices(
                subtask_annotations=subtask_annotations,
                current_rel_idx=current_rel_idx,
