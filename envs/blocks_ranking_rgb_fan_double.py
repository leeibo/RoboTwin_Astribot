...
class blocks_ranking_rgb_fan_double(Base_Task):
    ...
    LAYER_SPECS = {
        "lower": {
            ...
            "max_cyl_r": 0.5,
            "theta_shrink": 0.90,
            # Keep movable blocks on the same (left-arm) side as the target row.
            # Randomly spawning them on the opposite side made many seeds fail
            # during cross-body place attempts.
            "theta_min_deg": 2.0,
            "theta_max_deg": 38.0,
        },
        "upper": {
    ...
    def _after_rotate_visibility_refresh(self, visibility_map):
        ...
        if subtask_idx <= 0 or (not self._should_search_lower_before_upper_for_subtask(subtask_idx)):
            return None
        current_layer = normalize_layer(getattr(self, "search_cursor_layer", "lower") or "lower")
    ...
    def play_once(self):
        ...
        for pick_idx, place_idx, key, focus_key in [(1, 2, "B", "A")]:
            ...
            maybe_reset_head_for_subtask(self, place_idx, prev_subtask_idx=prev_subtask_idx)
            found_focus = search_focus(self, place_idx)
