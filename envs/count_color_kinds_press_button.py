...
class count_color_kinds_press_button(RMBenchButtonMixin, PolarCountLayoutMixin, Base_Task):
    """Count how many unique block colors appear, then press the 1/2/3 button."""
    ...
    def setup_demo(self, **kwargs):
        ...
        override = kwargs.pop("count_color_kind_override", None)
        self.unique_count_override = None if override is None else int(override)
        super()._init_task_env_(**kwargs)

    def _make_block(self, key, pose, color, label):
        block = create_box(scene=self, pose=pose, half_size=(self.BLOCK_HALF_SIZE,) * 3, color=color, name=str(key))
        block.set_mass(0.03)
    ...
    def _sample_color_labels(self):
        labels = [label for label, _ in self.COLOR_SPECS]
        unique_count = int(np.random.randint(1, 4) if self.unique_count_override is None else np.clip(self.unique_count_override, 1, 3))
        chosen = list(np.random.choice(labels, size=unique_count, replace=False))
        labels_for_blocks = chosen[:]
        while len(labels_for_blocks) < self.BLOCK_COUNT:
            labels_for_blocks.append(str(np.random.choice(chosen)))
        np.random.shuffle(labels_for_blocks)
        return unique_count, labels_for_blocks
    ...
    def load_actors(self):
        ...
        self.target_count, labels_for_blocks = self._sample_color_labels()
        self.appeared_colors = sorted(set(labels_for_blocks))
        color_map = dict(self.COLOR_SPECS)
        slots = self._sample_polar_count_slots(self.BLOCK_COUNT)
        self.blocks = {}
        self.block_color_labels = {}
        for idx, ((r, theta), label) in enumerate(zip(slots, labels_for_blocks), start=1):
            key = f"B{idx}"
            self.blocks[key] = self._make_block(key, self._block_pose_from_cyl(r, theta), color_map[label], f"{label} block")
            self.block_color_labels[key] = label
        self._make_buttons()
        self._configure_rotate_subtask_plan()

    def _configure_rotate_subtask_plan(self):
        registry = {**self.blocks, **{f"BTN{value}": button for value, button in self.buttons.items()}}
        target_button_key = f"BTN{int(self.target_count)}"
        self.configure_rotate_subtask_plan(
            object_registry=registry,
            subtask_defs=[
                dict(id=1, name="count_unique_block_colors", instruction_idx=1,
                     search_target_keys=list(self.blocks), action_target_keys=[], required_carried_keys=[],
                     carry_keys_after_done=[], allow_stage2_from_memory=False,
                     done_when="unique_color_count_known", next_subtask_id=2),
                dict(id=2, name="press_matching_number_button", instruction_idx=2,
                     search_target_keys=[target_button_key], action_target_keys=[target_button_key], required_carried_keys=[],
                     carry_keys_after_done=[], allow_stage2_from_memory=True,
                     done_when="matching_number_button_pressed", next_subtask_id=-1),
            ],
            task_instruction=(
                "Count how many unique block colors appear, then press the matching button; "
    ...
    def _scan_all_blocks_for_color_kinds(self):
        self._scan_count_objects_fixed_head(
            subtask_idx=1,
            target_keys=list(self.blocks),
            all_keys=list(self.blocks),
            objects=list(self.blocks.values()),
        )
    ...
    def play_once(self):
        self._scan_all_blocks_for_color_kinds()
    ...
    def _build_info(self):
        return {
            "{A}": "appeared block colors",
            "{B}": f"number {int(getattr(self, 'target_count', 0))} button",
            "{C}": ", ".join(getattr(self, "appeared_colors", [])),
