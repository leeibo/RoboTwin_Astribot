...
# DIFFICULTY_PRESETS="easy:150:1.0,medium:200:1.0,hard:240:1.0" ./collect_rotate_tasks_multi_difficulty.sh demo_clean_fan_double 0

# Batch collect rotate-view tasks across multiple difficulty levels.
#
# Usage:
#   bash collect_rotate_tasks_multi_difficulty.sh [BASE_CONFIG] [GPU_ID]
#
# Example:
#   DRY_RUN=1 bash collect_rotate_tasks_multi_difficulty.sh demo_clean 0
#   DRY_RUN=1 bash collect_rotate_tasks_multi_difficulty.sh demo_clean_fan_double 0
#
# Difficulty presets format:
#   DIFFICULTY_PRESETS="label:fan_angle_deg:shared_ratio,label2:fan:ratio"
# Default:
#   easy:150:0.85,medium:200:1.0,hard:240:1.0
...
DIFFICULTY_PRESETS="${DIFFICULTY_PRESETS:-easy:150:0.85,medium:200:1.0,hard:240:1.0}"
...
for item in "${PRESET_ITEMS[@]}"; do
  ...
  # Keep config name concise and deterministic.
  fan_tag="${fan_angle//./p}"
...
for config_name in "${GENERATED_CONFIGS[@]}"; do
  ...
  if CONTINUE_ON_ERROR="${CONTINUE_ON_ERROR}" DRY_RUN="${DRY_RUN}" \
      bash collect_rotate_tasks_except_fixing.sh "${config_name}" "${GPU_ID}"; then
